...
  //Use req.body for post requests
  ...
  var query2 = 'select round(avg(t.total), 2) as total, round(avg(t.fareAmt), 2) as fare, round(avg(t.tip), 2) as tip, round(100*avg(t.tip)/ avg(t.fareAmt), 2) as tippct, round(avg(t.dist), 2) as distance, round(avg(MINUTE(TIMEDIFF(t.timeDropoff,t.timePickup))), 2) as time from CIS550.Trip t inner join (select d from CIS550.Weather where snowfall between ' + req.params.snow_min + ' and ' + req.params.snow_max + ' and precipitation - snowfall between ' + req.params.rain_min + ' and ' + req.params.rain_max + ' and wind between ' + req.params.wind_min + ' and ' + req.params.wind_max + ' and temp between ' + req.params.temp_min + ' and ' + req.params.temp_max + ') as w on t.timePickup between w.d and DATE_ADD(w.d, INTERVAL 1 HOUR);'
 
  // var query = 
